...
class Cookie
{
    ...
    public static function set($name, $value, $expiresOrOptions = 0)
    {
        ...
        $_COOKIE[$name] = $value;
        Response::instance()->setCookie($name, $value, $expiresOrOptions, self::$path, self::$domain, self::$secure, self::$httponly);
    }
    ...
    {
        $name = self::$prefix . $name;
        Response::instance()->setCookie($name, null, -1, self::$path, self::$domain, self::$secure, self::$httponly);
        unset($_COOKIE[$name]);
    }
}
